...
float determinant(float array[N][N],float order)
{
    ...
    for(int i=0;i<ord-1;i++)
        for(int j=0;j<ord;j++)
            arr[ord-i-1][j]-=arr[ord-i-2][j];
    for(int i=0;i<ord;i++)
        for(int j=0;j<ord;j++)
            arr[ord-j-1][ord-i-2]+=arr[ord-j-1][ord-i-1];
    float det=1;
    for(int i=0;i<ord;i++)
        det*=arr[i][i];
    return det;
...
void generateB(float B[N][N],float cipher_text[N][N],float invkey[N][N],int rows,int cols)
{
    for(int i=0;i<rows;i++)
    {
        for(int j=0;j<cols;j++)
...
int main() {
	...
	// Please add your code here
        ...
        {
            for(int j=0;j<columns;j++)
                {mapped_decrypted_text[k]=matrixB[i][j];k+=1;}
        }
        mapped_decrypted_text[k]='\0';
        //cout<<mapped_decrypted_text<<endl;
        char decrypt_plain_text_zero_padded[ARRAY_SIZE];
        inv_array_map(mapped_decrypted_text,decrypt_plain_text_zero_padded);
        //cout<<decrypt_plain_text_zero_padded<<endl;
        /*int len;
        for(len=0;decrypt_plain_text_zero_padded[len];len++);*/
        int i;
        for(i=0;i<l1;i++)
